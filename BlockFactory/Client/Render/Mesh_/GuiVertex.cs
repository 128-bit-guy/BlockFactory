...
public struct GuiVertex
{
    ...
    public GuiVertex(Vector3D<float> pos, Vector4D<float> color, Vector2D<float> uv)
    {
        Pos = pos;
        Color = color;
        Uv = uv;
    ...
    public GuiVertex(float x, float y, float z, float r, float g, float b, float a, float u, float v) :
