...
public class TextureArrayManager
{
    ...
    {
        if (_imageIndices.TryGetValue(location, out int value))
        {
            return value;
        }
        else
    ...
    public void Upload()
    {
        ...
        GL.TexStorage3D(TextureTarget3d.Texture2DArray, 4, SizedInternalFormat.Rgba8, Images[0].Width, Images[0].Height, Images.Count);
        for (int i = 0; i < Images.Count; ++i)
        {
            GL.TexSubImage3D(TextureTarget.Texture2DArray, 0, 0, 0, i, Images[0].Width, Images[0].Height, 1, PixelFormat.Rgba, PixelType.UnsignedByte, Images[i].Data);
        }
