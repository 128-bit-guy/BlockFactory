...
public class SlotClientWidget : InGameMenuClientWidget<SlotWidget>
{
    ...
    public override unsafe void UpdateAndRender()
    {
        ...
        Screen.DrawColoredRect(new Box2(Box.Min, (Box.Max.X, Box.Min.Y + 5)), 1.5f, (0, 0, 0, 1));
        Screen.DrawColoredRect(new Box2(Box.Min, (Box.Min.X + 5, Box.Max.Y)), 1.5f, (0, 0, 0, 1));
        Screen.DrawColoredRect(new Box2((Box.Min.X, Box.Max.Y - 5), Box.Max), 1.5f, (0, 0, 0, 1));
        Screen.DrawColoredRect(new Box2((Box.Max.X - 5, Box.Min.Y), Box.Max), 1.5f, (0, 0, 0, 1));
    ...
    private void OnMouseButton(MouseButton button, InputAction action, KeyModifiers modifiers)
    {
        ...
        {
            var widgetAction = 0;
            if (button == MouseButton.Button2) widgetAction |= 1;

            if ((modifiers & KeyModifiers.Shift) != 0) widgetAction |= 2;
