...
public class TextInputWidget : Widget
{
    ...
    public override void UpdateAndRender()
    {
        ...
        var textToRender = GetRenderText();
        Screen.Client.Matrices.Push();
        Screen.Client.Matrices.Translate(new Vector3(Box.Min.X + 10,
            Box.Center.Y - Textures.TextRenderer.GetStringHeight(textToRender) / 2 - 3, ZIndex + 0.5f));
        Screen.DrawText(textToRender, -1);
        Screen.Client.Matrices.Pop();
        Screen.DrawColoredRect(new Box2(Box.Min, (Box.Max.X, Box.Min.Y + 5)), ZIndex + 0.5f, (0.8f, 0.8f, 0.8f, 1));
        Screen.DrawColoredRect(new Box2(Box.Min, (Box.Min.X + 5, Box.Max.Y)), ZIndex + 0.5f, (0.8f, 0.8f, 0.8f, 1));
        Screen.DrawColoredRect(new Box2((Box.Min.X, Box.Max.Y - 5), Box.Max), ZIndex + 0.5f, (0.8f, 0.8f, 0.8f, 1));
        Screen.DrawColoredRect(new Box2((Box.Max.X - 5, Box.Min.Y), Box.Max), ZIndex + 0.5f, (0.8f, 0.8f, 0.8f, 1));
    }
    ...
    private void OnMouseButton(MouseButton button, InputAction action, KeyModifiers modifiers)
    {
        if (action == InputAction.Press)
        {
            ...
            else if (Screen.ActiveWidget == this) Screen.ActiveWidget = null;
        }
    }
