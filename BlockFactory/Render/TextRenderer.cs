...
public class TextRenderer : IDisposable
{
    ...
    private static unsafe int BakeFontBitmap(byte[] ttf, int offset, float pixelHeight, byte[] pixels, int pw,
        int ph,
        int firstChar, int numChars, StbTrueType.stbtt_bakedchar[] chardata)
    {
        fixed (byte* ttfPtr = ttf)
        {
            fixed (byte* pixelsPtr = pixels)
            {
                ...
                {
                    var result = StbTrueType.stbtt_BakeFontBitmap(ttfPtr, offset, pixelHeight, pixelsPtr, pw, ph,
                        firstChar,
                        numChars,
                        chardataPtr);
