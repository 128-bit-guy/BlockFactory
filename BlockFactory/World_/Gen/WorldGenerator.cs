...
namespace BlockFactory.World_.Gen
{
    public class WorldGenerator
    {
        ...
        public void UpgradeChunkToLevel(ChunkGenerationLevel level, Chunk chunk)
        {
            ...
            foreach (Vector3i oPos in new Box3i(chunk.Pos - new Vector3i(1), chunk.Pos + new Vector3i(1)).InclusiveEnumerable())
            {
        ...
        private void PlaceTopSoil(Chunk chunk) {
            ...
            foreach (var a in chunk.GetInclusiveBox().InclusiveEnumerable()) {
                if ((chunk.Neighbourhood.GetBlockState(a).Block == Blocks.Stone || chunk.Neighbourhood.GetBlockState(a).Block == Blocks.Dirt) && chunk.Neighbourhood.GetBlockState(a + new Vector3i(0, 1, 0)).Block == Blocks.Air)
                {
                    var oPos = a;
                    while(true)
                    {
                        ...
                        if (chunk.Neighbourhood.GetBlockState(oPos).Block == Blocks.Stone || chunk.Neighbourhood.GetBlockState(oPos).Block == Blocks.Dirt)
                        {
                            if (oPos == a)
                            {
