namespace BlockFactory.Registry_
{
        ...
        where T : IRegistryEntry
    {
        ...
        private int[] BuildPermutation(RegistryName[] order) {
            ...
            int cNewPos = order.Length;
            int[] permutation = new int[_names.Count];
            for (int i = 0; i < _names.Count; ++i) {
                RegistryName name = _names[i];
                if (poses.TryGetValue(name, out int index))
                {
                    permutation[i] = index;
                }
                else
